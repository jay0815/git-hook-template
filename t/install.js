...
const PRE_COMMIT = path.join(__dirname, 'pre-commit')
...
const getPointGitFolderPath = async (root) => {

  const pointGit = path.resolve(root, '.git');

  const stat = await fsStat(pointGit).catch();

  if (stat.isDirectory()) {
    return pointGit
  } else {
    console.error('.git is not exist');
    console.error('your need init a git project first');
    process.exit(1);
  }
...
(async () => {
  console.log(ROOT);
  const pointGitPath = await getPointGitFolderPath(ROOT);
  console.log(pointGitPath);
})()



// var git = getPointGitFolderPath(ROOT);
